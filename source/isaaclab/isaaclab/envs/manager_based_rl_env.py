...
class ManagerBasedRLEnv(ManagerBasedEnv, gym.Env):
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        with NVTXMarker.range("env_step:total"):
            # process actions
            with NVTXMarker.range("env_step:process_action"):
            ...
            # -- check terminations
            with NVTXMarker.range("env_step:termination"):
                self.reset_buf = self.termination_manager.compute()
                self.reset_terminated = self.termination_manager.terminated
                self.reset_time_outs = self.termination_manager.time_outs
            # -- reward computation
            with NVTXMarker.range("env_step:reward"):
                self.reward_buf = self.reward_manager.compute(dt=self.step_dt)

            # -- recorder manager operations (if active)
            if len(self.recorder_manager.active_terms) > 0:
                with NVTXMarker.range("env_step:recorder_obs"):
                    # update observations for recording if needed
                    self.obs_buf = self.observation_manager.compute()
                with NVTXMarker.range("env_step:recorder_post_step"):
                    self.recorder_manager.record_post_step()

            # -- reset envs that terminated/timed-out and log the episode information
            reset_env_ids = self.reset_buf.nonzero(as_tuple=False).squeeze(-1)
            if len(reset_env_ids) > 0:
                with NVTXMarker.range("env_step:reset"):
                    # trigger recorder terms for pre-reset calls
                    with NVTXMarker.range("env_step:recorder_pre_reset"):
                        self.recorder_manager.record_pre_reset(reset_env_ids)

                    with NVTXMarker.range("env_step:reset_idx"):
                        self._reset_idx(reset_env_ids)

                    # if sensors are added to the scene, make sure we render to reflect changes in reset
                    if self.sim.has_rtx_sensors() and self.cfg.num_rerenders_on_reset > 0:
                        with NVTXMarker.range("env_step:reset_rerender"):
                            for _ in range(self.cfg.num_rerenders_on_reset):
                                self.sim.render()

                    # trigger recorder terms for post-reset calls
                    with NVTXMarker.range("env_step:recorder_post_reset"):
                        self.recorder_manager.record_post_reset(reset_env_ids)

            # -- update command
            with NVTXMarker.range("env_step:command"):
                self.command_manager.compute(dt=self.step_dt)
            # -- step interval events
            if "interval" in self.event_manager.available_modes:
                with NVTXMarker.range("env_step:events"):
                    self.event_manager.apply(mode="interval", dt=self.step_dt)
            # -- compute observations
            # note: done after reset to get the correct observations for reset envs
            with NVTXMarker.range("env_step:compute_observations"):
